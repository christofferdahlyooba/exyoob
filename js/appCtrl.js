...
/*
* The AngularJS controller 'AppCtrl'
*/
angular.module('mockApp').controller('AppCtrl', function($scope, $timeout){
	
    /*
    * Should be loaded automatically from the creator depending on login and appID or something
    */
	//The folder structure JSON
	...
	//The access and people settings JSON - Needs real users
	...
	//Load the JSON into the scope
	init($scope.savedSettings, $scope.folder, $scope.savedPeople);

	/*
	* Scope variables and functions for AppCtrl
	*/
	...
	/*
	* Activates select mode so that files and folders can be selected
	* Also unchecks all selected when deactivating select mode
	*/
	...
	/*
	* Calls checkFolders in mockApp.js to set that file or folders isChecked property
	*/
	...
	/*
	* If no favorite folder exists create it in rootFolder
	* then call addFavFolders
	*/
	...
	/*
	* Removes selected file or Folder from favorites
	*/
	...
	/*
	* Adds a file or folder to the favorite folder.
	* also adds view and sync access to user 0 so that they can be seen from the app view
	*/
	...
	/*
	* Returns true if the file or folder is in someones specified access list
	*/
	...
	/*
	* Shares the selected file or folder
	* Needs a call to native code to open mail or other share application
	*/
	...
	/*
	* Syncs the selected file or folder
	* Currently only sets a sync access to simulate beeing downloaded
	* needs to download the file or folder to device
	*/
	$scope.sync = function (node) {
		var answer = confirm('Do you want to sync ' + node.name + '?');
		if (answer) {
			//Download or sync the node here
			var user = 0;
...
/*
* Imports access and people settings
*/
